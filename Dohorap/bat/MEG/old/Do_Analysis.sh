...
#		mne_writeevents.sh $subject $block _ss
		...
		eveFile=${EVEDIR}${subject}/block${block}-eh-nocomments.eve
		for at in $AVRTYPES; do
			configFile=${CFGDIR}mne_dohorap${at}.cfg
			filteredFIF=${rawFIF}${filter}${at}_average-raw
			mne_average_raw.sh ${gconfigFile} ${configFile} ${rawFIF}.fif ${eveFile} ${filteredFIF}.fif > ${DOCDIR}mne_average_raw.log
