...
for groupID, group in enumerate(groups):
	...
	statText = 'Conditition difference found in {} from {}-{}, between {:1.3f}s and {:1.3f}s: p = {:2.1f}%, t({}) = {:1.2f}\n'
	for hem, hemisphere in enumerate(hemispheres):
		for loc, location in enumerate(locations):
			i = 0
			pValues = []
			...
			nValues = []
			for t, timewindow in enumerate(intervalTimewindows):
				objData = np.squeeze(subjectMean[:,0,loc,hem,metricID,t])
				subjData = np.squeeze(subjectMean[:,1,loc,hem,metricID,t])
				n = len(objData);
				...
				i += 1
			#pCorrected = pValues
			pCorrected = stats.p_adjust(FloatVector(pValues), method = 'fdr')
			i = 0
			for t, timewindow in enumerate(intervalTimewindows):
				...
				n = nValues[i]
				if pValue < 0.1:
					intervalStatFile.write(statText.format(group,location,hemisphere,tStart,tStop,pValue*100,n,tValue))
				i += 1
