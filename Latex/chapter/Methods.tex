...
The TRENtool software was used for exploring entropy transfers between cortical areas.
...
For each of these intervals, mean localized activity was computed from each cortical region.
...
Comparisons between these regions were performed for all regions along each of three pathways (see Fig. \ref{3.4.regions})
The dorsal pathway I connects PAC and BA6v, yielding exactly one comparison ($PAC_{lh} \rightarrow BA6v_{lh}$).
The dorsal pathway II connects PAC, pSTS and BA44, yielding 3 unique comparisons ($PAC_{lh} \rightarrow pSTS_{lh}$, $PAC_{lh} \rightarrow BA44_{lh}$, $pSTS_{lh} \rightarrow BA44_{lh}$).
The ventral pathway connects PAC, aSTG and BA45, yielding 3 unique comparisons ($PAC_{lh} \rightarrow aSTG_{lh}$, $PAC_{lh}-BA45_{lh}$, $aSTG_{lh} \rightarrow BA45_{lh}$).
PAC and pSTS showed effects across both hemispheres.
Therefore, I included comparisons across hemispheres for each of these regions (2 comparisons, $PAC_{lh} \rightarrow PAC_rh$ and $pSTS_{lh} \rightarrow pSTS_rh$) and their direct neighbors (3 comparisons, $PAC_rh \rightarrow pSTS_{lh}$, $PAC_{lh} \rightarrow pSTS_rh$, $pSTS_rh \rightarrow BA44_{lh}$).
Finally, for each of these 11 comparisons, the reverse direction was included as well.
